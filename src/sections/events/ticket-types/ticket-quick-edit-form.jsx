...
function TicketQuickEditForm({ currentTicket, open, onClose }) {
  ...
  const defaultValuesProfile = useMemo(
    () => ({
      ...
      status: currentTicket?.status || 'draft',
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      editTicket({ ...data, userId: currentTicket?.id });
...
export default TicketQuickEditForm;
