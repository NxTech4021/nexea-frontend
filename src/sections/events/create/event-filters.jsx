...
const CustomToolbar = ({ isFilterExpanded, setIsFilterExpanded, filters, theme, textColor }) => {
    ...
    >
      ...
      >
        ...
        <Button
          ...
          startIcon={
            <Iconify icon={isFilterExpanded ? 'eva:chevron-up-outline' : 'eva:funnel-outline'} />
          }
...
const EventFilters = ({
  ...
  const getUniqueTicketTypes = () => {
    ...
    const types = [
      ...new Set(
        data.attendees.map((attendee) => attendee.ticket.ticketType.title).filter(Boolean)
      ),
    ];
    return types.sort();
  };
  ...
  const getUniqueAddOns = () => {
    ...
    const addOns = [
      ...new Set(
        data.attendees.map((attendee) => attendee.ticket.ticketAddOn?.addOn?.name).filter(Boolean)
      ),
    ];
  ...
  const getUniqueDiscountCodes = () => {
    ...
    const codes = [
      ...new Set(
        data.attendees.map((attendee) => attendee.order.discountCode?.code).filter(Boolean)
      ),
    ];
  ...
  const filterOptions = [
    ...
    {
      field: 'status',
      label: isMobile ? 'Status' : 'Check-in Status',
      type: 'select',
      options: ['checkedIn', 'pending'],
    },
    ...
    {
      field: 'ticketType',
      label: isMobile ? 'Ticket' : 'Ticket Type',
      type: 'select',
      options: getUniqueTicketTypes(),
    },
    {
      field: 'ticketAddOn',
      label: isMobile ? 'Add-on' : 'Ticket Add On',
      type: 'select',
      options: getUniqueAddOns(),
    },
    { field: 'ticketCode', label: isMobile ? 'Code' : 'Ticket Code', type: 'text' },
    { field: 'orderNumber', label: isMobile ? 'Order #' : 'Order Number', type: 'text' },
    {
      field: 'orderStatus',
      label: isMobile ? 'Order' : 'Order Status',
      type: 'select',
      options: ['paid', 'free'],
    },
    {
      field: 'ticketStatus',
      label: isMobile ? 'Ticket' : 'Ticket Status',
      type: 'select',
      options: ['paid', 'free'],
    },
    {
      field: 'addOnStatus',
      label: isMobile ? 'Add-on' : 'Add-on Status',
      type: 'select',
      options: ['paid', 'free', 'none'],
    },
    {
      field: 'discountCode',
      label: isMobile ? 'Discount' : 'Discount Code',
      type: 'select',
      options: getUniqueDiscountCodes(),
    },
  ];
  ...
  useEffect(() => {
    ...
    let filtered = data.attendees.map((attendee) => ({
      ...
      orderStatus:
        attendee.order.status === 'paid' && Number(attendee.order.totalAmount) === 0
          ? 'free'
                  ...
                  {isMobile ? (
                    <Box sx={{ display: 'flex', gap: 1 }}>
                      ...
                      <FormControl size="small" sx={{ flex: 2, minWidth: 120 }}>
                        <Select
                          value={filter.field}
                          onChange={(e) =>
                            updateFilter(filter.id, { field: e.target.value, value: '' })
                          }
                          sx={{
                            height: 44,
                            fontSize: '0.875rem',
                    ...
                    <>
                      ...
                      <FormControl size="small" sx={{ minWidth: 80 }}>
                        ...
                        >
                          <MenuItem value="contains" sx={{ fontSize: '0.8125rem' }}>
                            contains
                          </MenuItem>
                          <MenuItem value="equals" sx={{ fontSize: '0.8125rem' }}>
                            equals
                          </MenuItem>
                          <MenuItem value="startsWith" sx={{ fontSize: '0.8125rem' }}>
                            starts with
                          </MenuItem>
                          <MenuItem value="endsWith" sx={{ fontSize: '0.8125rem' }}>
                            ends with
                          </MenuItem>
                        </Select>
                      </FormControl>
                        ...
                        return (
                          <FormControl fullWidth size="small">
                            ...
                            >
                              <MenuItem
                                value=""
                                sx={{ fontStyle: 'italic', color: secondaryTextColor }}
                              >
