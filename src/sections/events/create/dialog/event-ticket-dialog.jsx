...
import { createTicketType, editTicketType, deleteTicketType } from 'src/api/ticket-type';
...
const EventTicketDialog = ({ open, onClose, tickets: initialTickets, event }) => {
  ...
  const {
    ...
    error: eventError,
    mutate,
  } = useSWR(open && event?.id ? endpoints.events.detail(event.id) : null, fetcher);
  ...
  const handleSubmitTicket = methods.handleSubmit(async (data) => {
    try {
      ...
      if (isEditing && currentTicket?.id) {
        // Update existing ticket using the correct endpoint for editing
        await axiosInstance.put(endpoints.ticketType.edit(currentTicket.id), ticketData);
      ...
      // Close just the create/edit ticket dialog
        ...
        setTickets((prev) =>
          prev.map((ticket) =>
            ticket.id === currentTicket.id
              ? { ...ticket, ...ticketData, title: ticketData.title }
              : ticket
          )
        );
      } else {
      ...
      enqueueSnackbar(error.message || 'Failed to process ticket', { variant: 'error' });
    } finally {
      setIsLoading(false);
    }
  });
  ...
  const handleEditTicket = (ticket) => {
    ...
    methods.reset({
      ...
      validity: ticket.validity || null,
      requirement: {
        minimumTicketPerOrder: ticket.minimumTicketPerOrder || 1,
        maximumTicketPerOrder: ticket.maximumTicketPerOrder || 10,
      },
    });
  ...
  // Handle toggle ticket status
  const handleToggleStatus = async (ticket) => {
    try {
      setIsLoading(true);
      const newStatus = !ticket.isActive;

      //await axiosInstance.patch(`/api/tickets/toggle/${ticket.id}`);
      await axiosInstance.patch(endpoints.tickets.toggle(ticket.id), {
        isActive: newStatus,
      });

      // Optimistically update UI
      setTickets((prev) =>
        prev.map((t) => (t.id === ticket.id ? { ...t, isActive: newStatus } : t))
      );

      enqueueSnackbar(`Ticket ${newStatus ? 'activated' : 'deactivated'} successfully!`, {
        variant: 'success',
      });
      ...
      enqueueSnackbar(error.message || 'Failed to update ticket status', { variant: 'error' });
    } finally {
      setIsLoading(false);
    }
  };
                  ...
                  <Table sx={{ minWidth: 800 }}>
                    ...
                    <TableBody>
                      {tickets.map((ticket, index) => (
                        <TableRow
                          key={
                            ticket.id || `ticket-${ticket.title || ticket.name}-${Math.random()}`
                          }
                          sx={{
                            '&:hover': {
                              backgroundColor: (theme) =>
                            ...
                            borderColor: (theme) =>
                              theme.palette.mode === 'light' ? '#edf2f7' : '#2d3748',
                          }}
                        >
                          <TableCell
