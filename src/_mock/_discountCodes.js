...
function generateDiscountCodes(num) {
  const types = ['Fixed Amount (per item)', 'Fixed Amount (per order)', 'Percentage (%)'];
  ...
  for (let i = 0; i < num; i++) {
    ...
    const endDate = new Date(
      startDate.getTime() + Math.floor(Math.random() * 10000000000) + 86400000
    ); // End date is 1 day after start date
